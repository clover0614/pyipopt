...
class IPOPTInterface(cyipopt.Problem):
    def __init__(self, x0, obj, cons, lb, ub, cl, cu, use_exact_hessian=False):
        """
        初始化IPOPT优化问题接口
...
def solve_ipopt(x0, obj = None, cons = None, lb = None, ub = None, cl = None, cu = None, options=None, use_exact_hessian=False):
    """
    使用IPOPT求解优化问题
    ...
    nlp = IPOPTInterface(x0, obj, cons, lb, ub, cl, cu, use_exact_hessian)
...
def solve_ipopt_test(x0, func, returnsuccess=False):
    obj = lambda x: 0.0
    obj_grad = lambda x: np.zeros_like(x)
    obj_hess = lambda x, lagrange=None, obj_factor=None: np.zeros((len(x), len(x)))
    # constraints
    jac = jacobian(func)
    hess = hessian(func)
    # hess = None
    cons = [
        {'type': 'eq', 'fun': func, 'jac': jac, 'hess': hess},
        # {'type': 'ineq', 'fun': con_ineq_jit, 'jac': con_ineq_jac, 'hess': con_ineq_hessvp}
    ]
    bnds = [(0, adnp.inf) for _ in range(x0.size)]
    res = minimize_ipopt(obj, jac=obj_grad, hess=obj_hess, x0=x0, bounds=bnds,
    ...
    func(res.x)
    if returnsuccess:
        return res.x, res.info, res.success
    else:
